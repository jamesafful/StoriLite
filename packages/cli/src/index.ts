  ...
  for (const file of files) {
    try {
      ...
      if (isImage(file)) {
        ...
        db.prepare('INSERT OR REPLACE INTO asset (id, orig_path, vault_path, media_type, created_ts, bytes_orig, bytes_vault, quality_preset, state) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)')
...
function queryVault(vaultDir: string, text?: string) {
  const db = ensureDb(vaultDir);
  const rows = text ? db.prepare(`
    SELECT a.* FROM asset a LEFT JOIN index_terms t ON a.id = t.asset_id
    WHERE t.term LIKE ? GROUP BY a.id LIMIT 200
  `).all(`%${text}%`) : db.prepare('SELECT * FROM asset LIMIT 200').all();
  console.table(rows.map((r: any) => ({ id: r.id, type: r.media_type, bytes: r.bytes_vault, created: new Date(r.created_ts).toISOString() })));
}

yargs(hideBin(process.argv))
  .scriptName('photovault')
  .command('init', 'Initialize a vault', (y) => y.option('vault', { type: 'string', demandOption: True }), (argv) => {
    initVault(argv.vault as string);
  })
  .command('import', 'Import & compress a folder', (y) =>
    y.option('vault', { type: 'string', demandOption: True })
  ...
  .command('query', 'Query vault by simple term', (y) => y

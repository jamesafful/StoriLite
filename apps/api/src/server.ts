...
if (ffmpegPath) {
  // @ts-ignore fluent-ffmpeg path setter
...
async function main() {
  ...
  app.post('/api/compress', async (req) => {
    ...
    for (const file of files) {
      try {
        ...
        if (isImage(file)) {
          ...
          const ex: any = await extractExif(buf); // loosen typing to allow make/model
          db.prepare(
